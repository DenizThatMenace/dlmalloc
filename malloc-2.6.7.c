...
* Synopsis of compile-time options:
  ...
  MORECORE_CLEARS           (default 1)
     ...
     holds for sbrk).
  MORECORE_DISCRETE         (default: 0)
     If non-zero, the minimum amount of pages to request from MORECORE.
     Implies the returned area may not be from an increasing address as 
     per sbrk(), but in fact could be a smaller address than returned 
     previously by MORECORE.
  MALLOC_FAILURE_ACTION     (default: errno = ENOMEM)
     The action to take before "return 0" when malloc fails to be able 
     to return memory, either because memory is exhausted or because of
     illegal arguments.
...
/*
  The action to take on failure. By default, sets errno.
*/
...
/* 
  MORECORE_DISCRETE was introduced in part to make it easier to
  create a custom allocator on a macintosh. There are many ways to do
  this on a mac. Here is a sample version that you might be able to
  edit and use.  It uses virtually but not necessarily physically
  contiguous non-paged memory (locked in, present and won't get
  swapped out).  There is also a shutdown routine that should be
  called for cleanup.
  ...
  // called from nvrm_malloc() via the MORECORE macros
  void *osMoreCore(int size)
  {
    ...
    {
      // we don't currently support shrink behavior for MORECORE_DISCRETE
      return (void *) MORECORE_FAILURE;
    }
    else
    {
...
#ifndef MORECORE_DISCRETE
#define MORECORE_DISCRETE 0
#endif
...
#define request2size(req) \
 (((long)(req) < 0) ? -1 : \
  (((long)((req) + (SIZE_SZ + MALLOC_ALIGN_MASK)) < 0)? -1: \
...
#if MORECORE_DISCRETE
  if (sbrk_size < (MORECORE_DISCRETE * pagesz))
      sbrk_size = (MORECORE_DISCRETE * pagesz);
  sbrk_size += pagesz - 1;
  sbrk_size &= ~(pagesz - 1);
#endif
...
#if ! MORECORE_DISCRETE
  /* Fail if a foreign sbrk call backed up into our space */
  if (brk < old_end && old_top != initial_top)
    return;     
#endif
...
History:
    ...
    V?.?.? ??? ??? ?? ??:??:?? ????  Doug Lea  (dl at gee)
      * Introduce MALLOC_FAILURE_ACTION, MORECORE_DISCRETE.
