...
* PRELIMINARY VERSION 2.6.3a  Sun May 19 08:17:35 1996  Doug Lea  (dl at gee)
...
/*
  ...
  Realloc algorithm:
    ...
    Otherwise, if the reallocation is for additional space, and the chunk can be
    ...
    If the reallocation is for less space, and the new request
    is for a `small' (<512 bytes) size:

       * Unless the #define REALLOC_ZERO_BYTES_FREES
         is set, realloc with a size argument of zero (re)allocates a
         minimum-sized chunk.
       * If there is an available unused chunk that exactly
         fits the new size, it is moved (copied) there.
       * If the previous adjacent chunk is unused, it is 
         slided backward into this space to minimize fragmentation.

    In any case, newly unused space is lopped off and freed.
...
#endif
{
  ...
  /* if shrinking, replace with exact fit or slide back to cover hole */
  else if (is_small_request(nb) && oldsize - nb >= MINSIZE)
  {
    mchunkptr victim;
    size_t victim_size;
         ...
         ((victim = last(bin_at(idx+1))) != bin_at(idx+1)))
    {
      ...
      set_inuse_bit_at_offset(victim, victim_size);
      check_malloced_chunk(victim, nb);
      newmem = chunk2mem(victim);
      MALLOC_COPY(newmem, oldmem, nb - SIZE_SZ);
      fREe(oldmem);
      return newmem;
    }
...
History:
    ...
    V2.6.3a Sun May 19 08:17:58 1996  Doug Lea  (dl at gee)
      * Realloc: try exact fits and/or slide shrinking requests backwards
      * Malloc: add tiny optimization from Raymond Nijssen
